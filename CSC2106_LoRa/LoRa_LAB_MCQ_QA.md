...
<ol>
  <li>TX retries until max even though RX prints payload. Most likely mismatch is:<br>
    A. ACK destination/source fields not matching TX node ID expectation<br>
    B. OLED text size<br>
    C. RSSI too high<br>
    D. SPI include order<br>
  </li>
  <li>RX receives packet but labels checksum fail frequently. Best code area to inspect:<br>
    A. Checksum function field order and payloadLen boundaries<br>
    B. OLED reset pin<br>
    C. Serial baud<br>
    D. Delay duration<br>
  </li>
  <li>Many irrelevant packets shown in crowded channel. Assignment-level mitigation is:<br>
    A. Ignore destID checks<br>
    B. ID-based filtering by destination<br>
    C. Remove checksum<br>
    D. Reduce payloadLen to zero<br>
  </li>
  <li>TX often reports timeout in noisy environment. Most robust improvement is:<br>
    A. Add retry/backoff and tune ACK timeout<br>
    B. Disable ACK<br>
    C. Remove waitPacketSent<br>
    D. Lower node IDs<br>
  </li>
  <li>OLED blank but serial indicates radio works. Most likely issue is:<br>
    A. Display config mismatch or init failure handling path<br>
    B. Frequency mismatch<br>
    C. Wrong checksum<br>
    D. ACK type constant<br>
  </li>
</ol>

### C2. Short-Answer Bank

1. Why include payloadLen in both packet and checksum logic?
2. Why waitPacketSent is important after rf95.send?
3. Why is checksum alone not a security mechanism?
4. Why can identical node IDs cause reliability issues?
5. Difference between ACK and NACK branch purpose?
...
### Section C2
1. Receiver needs payload boundary for safe parse, and checksum must include length to detect truncation/length tampering.
2. It blocks until radio transmission completes, preventing immediate logic transitions that could disrupt timing assumptions.
3. It detects accidental corruption but does not authenticate sender or protect confidentiality against intentional tampering.
4. ACK/addressing decisions rely on node identity; duplicates create ambiguous routing and false ACK interpretation.
5. ACK confirms valid delivery/parse, NACK signals detected corruption so sender can retry with awareness of failure type.
